...
class NWB_adapter(object):
    ...
    def get_wm(self, probename, bin_num=20, wm=0):
        ...
        if wm==0:
            ...
            a = plt.hist(channel_ypos,bins=bin_num)
    ...
    def get_unit_list(self, probename):
        ...
        self.channel_list = self.nwb_data['processing'][probename]['channel_list'].value
    ...
    def get_matrix_V1(self, key, pre_time=0., post_time=0., wm_given={}, stim_table=[], fs=1000):
        ...
        wm_given: dictionary of 'probe': wm
        stim_table: modified stim_table if different from direct key fetch
    ...
    def get_matrix_V1_sortch(self, key, pre_time=0., post_time=0., wm_given={}, stim_table=[], probenames=[], fs=1000):
        ...
        # cancatenate all data from different probes
        if len(probenames)==0:
            probenames = np.array(list(self.nwb_data['processing'].keys())).astype(str)
    ...
    def get_matrix_V1_depth(self, key, wm_given={}, pre_time=0., post_time=0., fs=1000):
        """Create response matrix for given stimulus condition (key).
        Matrix is digitized between ['Start']-pre_time and ['Start']+post_time."""
        # cancatenate all data from different probes
        probenames = list(self.nwb_data['processing'].keys())
